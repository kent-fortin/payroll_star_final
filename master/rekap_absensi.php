          ...
          while ($row = mysqli_fetch_assoc($data)): ?>
            <tr>
              ...
              <td>
                ...
                <span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle px-2 py-1 mt-1"><i
              ...
              <td>
                <div class="d-flex flex-wrap gap-1" style="max-width: 220px;">
                  <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1"><i
                      class="bi bi-check-circle me-1"></i>Hadir: <strong>
                      <?= $row['hadir'] ?>
                    </strong></span>
                  <span class="badge bg-warning-subtle text-warning border border-warning-subtle px-2 py-1"><i
                      class="bi bi-plus-circle me-1"></i>Sakit: <strong>
                      <?= $row['sakit'] ?>
                    </strong></span>
                  <span class="badge bg-info-subtle text-info border border-info-subtle px-2 py-1"><i
                      class="bi bi-info-circle me-1"></i>Izin: <strong>
                      <?= $row['izin'] ?>
                    </strong></span>
                  <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1"><i
                      class="bi bi-x-circle me-1"></i>Alpha: <strong>
                      <?= $row['alpha'] ?>
                    </strong></span>
                </div>
              </td>
              <td>
                ...
                <div class="<?= $pot > 0 ? 'text-danger fw-bold fs-6' : 'text-success fw-semibold' ?>">
                  <?= rupiah($pot) ?>
                </div>
              ...
              <td>
                ...
                <?php if (!empty($row['catatan_pimpinan'])): ?>
                  <div class="mt-2 p-2 bg-danger-subtle text-danger rounded border border-danger-subtle small fw-semibold"
                    style="max-width: 220px; word-wrap: break-word; white-space: normal; font-size: 0.75rem;">
              ...
              <td class="text-center">
                <?php if ($row['status_edit'] === 'Menunggu'): ?>
                  <button class="btn btn-sm btn-outline-secondary w-100 py-2 fw-semibold" disabled
                    title="Menunggu Persetujuan Pimpinan"><i class="bi bi-hourglass-split me-1"></i> Menunggu</button>
                <?php else: ?>
                  <button type="button" class="btn btn-sm btn-outline-primary w-100 py-2 fw-semibold" data-bs-toggle="modal"
                    data-bs-target="#editModal<?= $row['id_absensi'] ?>" title="Ajukan Edit">
                  ...
                  <div class="modal fade" id="editModal<?= $row['id_absensi'] ?>" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                      <form method="post" class="modal-content text-start form-ajukan-edit shadow-lg border-0"
                        ...
                        <div class="modal-header bg-light border-bottom py-3">
                          <h5 class="modal-title fs-6 fw-bold text-dark"><i
                              class="bi bi-pencil-square text-primary me-2"></i>Ajukan Edit Absensi -
                            <?= e($row['nama_karyawan']) ?>
                          </h5>
                        ...
                        <div class="modal-body p-4">
                          <div
                            class="d-flex justify-content-between align-items-center mb-3 p-2 bg-secondary-subtle rounded small text-secondary">
                            <span><strong>NIP:</strong>
                              <?= e($row['nip']) ?>
                            </span>
                          ...
                          <div class="row g-3 mb-4">
                            <div class="col-6">
                              <label class="form-label small text-success fw-semibold"><i
                                  class="bi bi-check-circle me-1"></i>Hadir</label>
                              <input type="number" name="hadir" class="form-control fw-bold" value="<?= $row['hadir'] ?>"
                                min="0" required>
                            </div>
                            <div class="col-6">
                              <label class="form-label small text-warning fw-semibold"><i
                                  class="bi bi-plus-circle me-1"></i>Sakit</label>
                              <input type="number" name="sakit" class="form-control fw-bold" value="<?= $row['sakit'] ?>"
                                min="0" required>
                            </div>
                            <div class="col-6">
                              <label class="form-label small text-info fw-semibold"><i
                                  class="bi bi-info-circle me-1"></i>Izin</label>
                              <input type="number" name="izin" class="form-control fw-bold" value="<?= $row['izin'] ?>"
                                min="0" required>
                            </div>
                            <div class="col-6">
                              <label class="form-label small text-danger fw-semibold"><i
                                  class="bi bi-x-circle me-1"></i>Alpha</label>
                              <input type="number" name="alpha" class="form-control fw-bold" value="<?= $row['alpha'] ?>"
                                min="0" required>
                            </div>
                          </div>

                          <div class="mb-2">
                            <label class="form-label fw-bold small text-dark">Alasan Perubahan <span
                                class="text-danger">*</span></label>
                            <textarea name="alasan" class="form-control" rows="3" required
                              placeholder="Contoh: Koreksi salah input hari alpha karena karyawan ternyata izin cuti tahunan..."></textarea>
                        ...
                        <div class="modal-footer bg-light border-top py-2">
                          ...
                          <button type="submit" class="btn btn-sm btn-primary px-4 fw-bold"><i
                              class="bi bi-send me-1"></i>Kirim Pengajuan</button>
                        </div>
                      </form>
                    </div>
                  </div>
...
<div class="card p-4 mt-4">
  ...
  <p class="section-desc mb-4">Daftar seluruh riwayat pengajuan edit absensi kepada Pimpinan beserta status dan catatan
    keputusannya.</p>
  <div class="table-responsive">
    <table class="table table-hover dt-table align-middle" style="width:100%">
      <thead>
            ...
            <tr>
              ...
              <td>
                <div class="d-flex flex-wrap gap-1" style="max-width: 200px;">
                  <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1">H: <strong>
                      <?= $h['hadir_baru'] ?>
                    </strong></span>
                  <span class="badge bg-warning-subtle text-warning border border-warning-subtle px-2 py-1">S: <strong>
                      <?= $h['sakit_baru'] ?>
                    </strong></span>
                  <span class="badge bg-info-subtle text-info border border-info-subtle px-2 py-1">I: <strong>
                      <?= $h['izin_baru'] ?>
                    </strong></span>
                  <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1">A: <strong>
                      <?= $h['alpha_baru'] ?>
                    </strong></span>
                </div>
              </td>
              <td>
                ...
                <div class="small text-muted" style="font-size: 0.75rem;"><i class="bi bi-clock me-1"></i>
                  <?= e($h['tanggal_pengajuan']) ?>
                </div>
              </td>
              <td>
                ...
                <?php if (!empty($h['catatan_pimpinan'])): ?>
                  <div
                    class="mt-2 p-2 <?= $h['status'] === 'Ditolak' ? 'bg-danger-subtle text-danger border border-danger-subtle' : 'bg-light text-dark border' ?> rounded small fw-semibold"
                    style="max-width: 220px; white-space: normal !important; word-break: break-word; font-size: 0.75rem;">
                    <i class="bi bi-chat-left-text me-1"></i>
                    <?= e($h['catatan_pimpinan']) ?>
                  </div>
                <?php endif; ?>
              </td>
            </tr>
