...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['simpan'])) {
    ...
    $hadir=max(0,(int)($_POST['hadir']??0)); $sakit=max(0,(int)($_POST['sakit']??0)); $izin=max(0,(int)($_POST['izin']??0)); $alpha=max(0,(int)($_POST['alpha']??0)); $lembur=max(0,(int)($_POST['lembur_jam']??0));
    if($idKaryawan<1||bulan_nomor($bulan)===0||$tahun<2000){
        ...
        if(!$old){
            ...
            $json=mysqli_real_escape_string($conn,json_encode(['hadir'=>$old['hadir'],'sakit'=>$old['sakit'],'izin'=>$old['izin'],'alpha'=>$old['alpha'],'lembur_jam'=>$old['lembur_jam']],JSON_UNESCAPED_UNICODE));
            $alasanEsc=mysqli_real_escape_string($conn,$alasan); $userId=(int)$_SESSION['id_user'];
            $sql="INSERT INTO permintaan_edit_absensi (id_absensi,hadir_baru,sakit_baru,izin_baru,alpha_baru,lembur_jam_baru,alasan_perubahan,data_lama,id_pengaju) VALUES ($idAbsensi,$hadir,$sakit,$izin,$alpha,$lembur,'$alasanEsc','$json',$userId)";
            $ok=mysqli_query($conn,$sql);
            set_flash($ok?'success':'danger',$ok?'Permintaan edit absensi telah dikirim kepada pimpinan.':'Permintaan edit absensi gagal dikirim.');
            if(!$ok) app_log('Request edit absensi: '.mysqli_error($conn));
        }
    } else {
        $bulanEsc=mysqli_real_escape_string($conn,$bulan); $userId=(int)$_SESSION['id_user'];
        $stmt=mysqli_prepare($conn,'INSERT INTO absensi (id_karyawan,bulan,tahun,hadir,sakit,izin,alpha,lembur_jam,dibuat_oleh) VALUES (?,?,?,?,?,?,?,?,?)');
        $ok=false;
        if($stmt){mysqli_stmt_bind_param($stmt,'isiiiiiii',$idKaryawan,$bulan,$tahun,$hadir,$sakit,$izin,$alpha,$lembur,$userId);$ok=mysqli_stmt_execute($stmt);}
...
$karyawan=mysqli_query($conn,'SELECT id_karyawan,nip,nama_karyawan FROM karyawan ORDER BY nama_karyawan');
...
$tarifLembur=get_setting($conn,'tarif_lembur_per_jam',15000);$tarifAlpha=get_setting($conn,'potongan_alpha_per_hari',25000);
?>
...
<p class="section-desc"><?= $edit ? 'Edit data harus disetujui pimpinan.' : 'Data yang dicatat adalah rekap bulanan, bukan presensi harian.' ?></p>
...
<div class="col-md-1"><label class="form-label">Hadir</label><input type="number" min="0" name="hadir" class="form-control" value="<?= e($edit['hadir']??0) ?>"></div><div class="col-md-1"><label class="form-label">Sakit</label><input type="number" min="0" name="sakit" class="form-control" value="<?= e($edit['sakit']??0) ?>"></div><div class="col-md-1"><label class="form-label">Izin</label><input type="number" min="0" name="izin" class="form-control" value="<?= e($edit['izin']??0) ?>"></div><div class="col-md-1"><label class="form-label">Alpha</label><input type="number" min="0" name="alpha" class="form-control" value="<?= e($edit['alpha']??0) ?>"></div>
<div class="col-md-3"><label class="form-label">Jumlah Jam Lembur</label><input type="number" min="0" name="lembur_jam" class="form-control" value="<?= e($edit['lembur_jam']??0) ?>"><div class="form-text">1 jam × <?= rupiah($tarifLembur) ?></div></div>
...
<div class="formula-box mb-4 small"><strong>Perhitungan otomatis:</strong> lembur = jam × <?= rupiah($tarifLembur) ?>; potongan alpha = hari alpha × <?= rupiah($tarifAlpha) ?>.</div>
<div class="card p-4">
<div class="section-header">
  <i class="bi bi-table"></i>
  <h2 class="h5">Daftar Rekap Absensi</h2>
</div>
<div class="table-responsive"><table class="table table-striped dt-table" style="width:100%"><thead><tr><th>No</th><th>NIP</th><th>Nama</th><th>Periode</th><th>Hadir</th><th>Sakit</th><th>Izin</th><th>Alpha</th><th>Jam Lembur</th><th>Nilai Lembur</th><th>Potongan Alpha</th><th>Status Edit</th><th>Aksi</th></tr></thead><tbody>
<?php $no=1;if($data):while($row=mysqli_fetch_assoc($data)):?><tr><td><?= $no++ ?></td><td><?= e($row['nip']) ?></td><td><?= e($row['nama_karyawan']) ?></td><td><?= e($row['bulan'].' '.$row['tahun']) ?></td><td><?= $row['hadir'] ?></td><td><?= $row['sakit'] ?></td><td><?= $row['izin'] ?></td><td><?= $row['alpha'] ?></td><td><?= $row['lembur_jam'] ?></td><td><?= rupiah($row['lembur_jam']*$tarifLembur) ?></td><td><?= rupiah($row['alpha']*$tarifAlpha) ?></td><td><?= $row['status_edit']?status_badge($row['status_edit']):'<span class="text-muted">-</span>' ?></td><td><a class="btn btn-sm btn-warning" href="?edit=<?= $row['id_absensi'] ?>">Ajukan Edit</a></td></tr><?php endwhile;endif;?>
<?php if(!$data||mysqli_num_rows($data)===0):?><tr><td colspan="13" class="text-center text-muted">Belum ada rekap absensi.</td></tr><?php endif;?></tbody></table></div></div>
