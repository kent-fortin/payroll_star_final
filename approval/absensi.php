...
<p class="section-desc mb-4">Pimpinan menyetujui atau menolak perubahan rekap absensi bulanan (Hadir, Sakit, Izin, Alpha) yang diajukan oleh Admin.</p>
      ...
      <tr>
          ...
          <div class="d-flex align-items-center gap-2" style="min-width: 200px;">
            <div class="bg-light p-2 rounded border flex-fill text-center" style="font-size: 0.8rem;">
              ...
              <div class="d-flex justify-content-around">
                <span title="Hadir" class="text-success fw-bold">H:<?= (int)($old['hadir']??0) ?></span>
                <span title="Sakit" class="text-warning fw-bold">S:<?= (int)($old['sakit']??0) ?></span>
                <span title="Izin" class="text-info fw-bold">I:<?= (int)($old['izin']??0) ?></span>
                <span title="Alpha" class="text-danger fw-bold">A:<?= (int)($old['alpha']??0) ?></span>
              </div>
            </div>
            <i class="bi bi-arrow-right-circle-fill text-primary fs-5"></i>
            <div class="bg-primary-subtle p-2 rounded border border-primary-subtle flex-fill text-center" style="font-size: 0.8rem;">
              <div class="text-primary fw-bold mb-1" style="font-size: 0.65rem; letter-spacing: 0.5px;">USULAN BARU</div>
              <div class="d-flex justify-content-around">
                <span title="Hadir" class="text-success fw-bold">H:<?= (int)$row['hadir_baru'] ?></span>
                <span title="Sakit" class="text-warning fw-bold">S:<?= (int)$row['sakit_baru'] ?></span>
                <span title="Izin" class="text-info fw-bold">I:<?= (int)$row['izin_baru'] ?></span>
                <span title="Alpha" class="text-danger fw-bold">A:<?= (int)$row['alpha_baru'] ?></span>
              </div>
            </div>
          </div>
        </td>
        ...
        <td>
          ...
          <?php if($row['catatan_pimpinan']): ?>
            <div class="mt-2 p-2 bg-danger-subtle text-danger rounded border border-danger-subtle small fw-semibold" style="max-width: 200px; word-wrap: break-word; white-space: normal; font-size: 0.75rem;">
        ...
        <td class="text-center">
          ...
          <form method="post" class="form-approval-absensi m-0">
            ...
            <div class="d-flex justify-content-center gap-1">
              <button type="button" class="btn btn-sm btn-success fw-bold px-3 btn-setujui-absensi" title="Setujui Usulan"><i class="bi bi-check-lg me-1"></i>Setujui</button>
              <button type="button" class="btn btn-sm btn-danger fw-bold px-3 btn-tolak-absensi" title="Tolak Usulan"><i class="bi bi-x-lg me-1"></i>Tolak</button>
            </div>
          </form>
          <?php else: ?>
